...
class PatientUser(db.Model):
    ...
    orders = db.relationship('Order', backref='patient_user', lazy='dynamic') #关联预约表
    consults = db.relationship('Consult', backref='patient_user', lazy='dynamic')     #关联用户咨询表
...
class Doctor(db.Model):
    ...
    orders = db.relationship('Order', backref='doctor', lazy='dynamic')     #关联病人预约表
    consults = db.relationship('Consult', backref='doctor', lazy='dynamic')     #关联用户咨询表
...
class Consult(db.Model):
    ...
    def __init__(self, theme, consult_time, content, reply, status):
...
class Order(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    patient_user_id = db.Column(db.Integer, db.ForeignKey('patient_user.id'))  # 关联用户id
    doctor_id = db.Column(db.Integer, db.ForeignKey('doctor.id'))  # 关联医生id
    status = db.Column(db.String(2))  # 预约时间是上午还是下午
    ordertime = db.Column(db.DATE, nullable=False)  # 预约时间
    painter_name = db.Column(db.String(30), nullable=False)  # 患者姓名
    disease = db.Column(db.String(150), nullable=False)  # 病情描述
    id_card = db.Column(db.String(18), nullable=False)  # 身份证号码
    sex = db.Column(db.String(6), nullable=False)  # 性别
    phone = db.Column(db.String(11), nullable=False)  # 手机号码
    addtime = db.Column(db.DateTime, nullable=False)  # 添加该记录的时间   

    def __init__(self, status, ordertime, painter_name, disease, \
                id_card, sex, phone, addtime):
        self.status = status
    ...
    def __repr__(self):
        return "<user:%r>" % (self.painter_name)